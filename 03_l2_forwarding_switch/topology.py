#!/usr/bin/env python3
"""Mininet topology for Case 03: static L2 forwarding.

Single switch with four hosts, each with a well-known MAC. The Go
controller installs dmac entries 00:00:00:00:00:0N -> port N. Because
the P4 program only handles unicast EXACT matches (no ARP broadcast),
every host's ARP table is pre-populated so pingAll works without
broadcast learning.
"""
...
def populate_arp(net: Mininet, n_hosts: int) -> None:
    """Install static ARP entries so pings don't need broadcast ARP."""
    ...
    for i in range(1, n_hosts + 1):
        ...
        for j in range(1, n_hosts + 1):
            if i == j:
                continue
            hi.cmd(f"arp -s 10.0.0.{j} {host_mac(j)}")


def run_controller(controller_bin: str, p4info: str, config: str, hosts: int) -> subprocess.Popen:
    info("*** Launching Go controller to push pipeline + dmac entries\n")
...
def wait_controller_ready(proc: subprocess.Popen, timeout: float = 15.0) -> bool:
    ...
    while time.time() < deadline:
        ...
        if "l2-forward ready" in decoded:
            return True
    return False
...
def main() -> None:
    ...
    setLogLevel("info")
    net = Mininet(topo=L2Topo(n_hosts=args.n_hosts), controller=None)
    net.start()
    populate_arp(net, args.n_hosts)
    ...
    try:
        if args.run_test:
            info("*** Running pingAll across the topology\n")
            dropped = net.pingAll(timeout="2")
            print(f"ping drop ratio: {dropped}%")
            rc = 0 if dropped == 0 else 1
            print("SUCCESS: full mesh reachable via dmac table" if rc == 0
