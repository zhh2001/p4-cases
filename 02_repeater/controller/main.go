// Case 02: port repeater controller.
//
// The repeater P4 program hard-codes the 1<->2 port mapping in ingress
// apply{} with an if/else, so the controller only needs to push the
// forwarding pipeline. No tables are declared.
package main
...
func main() {
	...
	fmt.Printf("pipeline installed via %s; repeater ready\n", res.Action)
