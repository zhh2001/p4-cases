#!/usr/bin/env bash
# Case 06: In-band Network Telemetry across 3 switches.
#
#   sudo ./run.sh        # automated send/receive test
...
( cd "${REPO_ROOT}" && go build -o "${BIN_DIR}/controller" ./06_int/controller )
...
log "Starting mininet + 3 controllers + (optional) send/receive test"
