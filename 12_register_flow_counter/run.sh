...
( cd "${REPO_ROOT}" && go build -o "${BIN_DIR}/controller" ./12_register_flow_counter/controller )

MODE="${1:-test}"
EXTRA=()
if [[ "${MODE}" == "test" ]]; then EXTRA+=(--run-test); fi

log "Starting mininet + controller + (optional) register read test"
